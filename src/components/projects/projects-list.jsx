...
export function ProjectsList() {
  ...
  // Safely load saved view preference after mount to avoid hydration mismatch
  useEffect(() => {
            ...
            <div className="flex items-center bg-slate-100 p-0.5 rounded-lg">
              <button
                ...
                className={`p-1.5 rounded-md text-xs font-medium flex items-center gap-1 transition ${
                  view === "grid" ? "bg-white text-violet-600 shadow-xs" : "text-slate-500 hover:text-slate-800"
                }`}
              >
              ...
              <button
                ...
                className={`p-1.5 rounded-md text-xs font-medium flex items-center gap-1 transition ${
                  view === "table" ? "bg-white text-violet-600 shadow-xs" : "text-slate-500 hover:text-slate-800"
                }`}
              >
              ...
              {upcomingProjects.map(project => (
                <div key={project.id} className="flex items-center justify-between border-b border-slate-100 pb-2.5 last:border-0 last:pb-0">
                  ...
                  <span 
                    ...
                    className="shrink-0 px-2.5 py-1 text-xs font-semibold bg-violet-50 text-violet-700 rounded-lg border border-violet-100"
                  >
                    {mounted ? new Date(project.endDate).toLocaleDateString() : ''}
                  </span>
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div className="xl:col-span-2 project-panel p-5 space-y-4">
          ...
          <div className="flex items-center justify-between border-b border-slate-100 pb-3 text-sm">
            <div className="flex gap-4 sm:gap-6 overflow-x-auto w-full">
              {[
            ...
            <>
              {view === "grid" ? (
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between px-4 py-2.5 rounded-xl border border-slate-200/80 bg-slate-50/60">
                    <p className="text-xs text-slate-500">
                      Showing <span className="font-semibold text-slate-700">{filteredProjects.length > 0 ? startIndex + 1 : 0}</span> to{' '}
                      <span className="font-semibold text-slate-700">
                        {Math.min(startIndex + itemsPerPage, filteredProjects.length)}
                      </span>{' '}
                      of <span className="font-semibold text-slate-700">{filteredProjects.length}</span> projects
                    </p>
                ...
                <div className="w-full">
                  <div className="w-full rounded-xl border border-slate-200/80 overflow-hidden">
                    <table className="w-full text-left border-collapse table-fixed">
                      <thead>
                        <tr className="border-b border-slate-200 bg-slate-50/80 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                          <th className="py-3 px-4 w-[35%]">Project Name</th>
                          <th className="py-3 px-4 w-[18%]">Manager</th>
                          <th className="py-3 px-4 w-[15%]">Team</th>
                          <th className="py-3 px-4 w-[15%]">Deadline</th>
                          <th className="py-3 px-4 w-[17%] text-right">Status</th>
                        </tr>
                      </thead>
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="py-3.5 px-4 text-right truncate">
                                ...
                                >
                                  <span className={`inline-block px-2.5 py-1 text-xs font-semibold border rounded-lg capitalize hover:opacity-80 transition ${statusStyles}`}>
