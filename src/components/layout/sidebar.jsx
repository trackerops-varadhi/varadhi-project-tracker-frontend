...
import {
  ...
  LogOut, ChevronLeft, ChevronRight, Bell, X,
...
export function Sidebar({ collapsed: collapsedProp, setCollapsed: setCollapsedProp, mobileDrawer = false, onNavigate }) {
  ...
  return (
    <aside
      className={cn(
        'rounded-r-2xl bg-white border-r border-slate-200 flex flex-col transition-all duration-300 ease-in-out',
        mobileDrawer ? 'relative h-full w-full' : 'fixed left-0 top-0 h-dvh z-30',
    ...
    >
      ...
      <div className="flex items-center pl-1 pr-4 py-5 border-b border-slate-100">
        ...
        <button
          type="button"
          onClick={mobileDrawer ? onNavigate : () => setCollapsed(!collapsed)}
          aria-label={mobileDrawer ? 'Close navigation' : collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
      ...
      {/* Nav Items */}
      <nav aria-label="Main navigation" className="min-h-0 flex-1 px-2 py-4 space-y-1 overflow-y-auto">
        ...
        {filteredNav.map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                'flex min-h-11 md:min-h-0 items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all relative',
                isActive
